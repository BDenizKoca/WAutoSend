...
window.WASDebug = {
    ...
    /**
     * Test post-send action functionality
     */
    async testPostSendActions() {
        console.log('=== Testing Post-Send Action Functionality ===');
        ...
        try {
            // Check current post-send action setting
            const settings = await window.wasStorage.getSettings() || {};
            const action = settings.postSendAction || 'refresh';
            console.log('Current post-send action setting:', action);
            
            switch (action) {
                case 'refresh':
                    console.log('✓ Refresh mode: Page will refresh after sending messages');
                    break;
                case 'close':
                    console.log('✓ Close mode: Tab will close after sending messages (avoids read confirmations & online status)');
                    break;
                case 'none':
                    console.log('✓ None mode: No action after sending messages');
                    break;
                default:
                    console.log('? Unknown mode, will default to refresh');
                    break;
            }
            
            console.log('Simulating a successful message send...');
            
            if (action === 'close') {
                console.warn('CLOSE TAB TEST: Tab would close in 5 seconds! This is just a test.');
            } else if (action === 'refresh') {
                console.warn('REFRESH TEST: Page would refresh in 5 seconds! This is just a test.');
            } else {
                console.log('NO ACTION TEST: Nothing would happen after sending.');
            }
            
            setTimeout(() => {
                console.log('Test completed - post-send action functionality is working');
                console.log('In real usage, the action would execute here.');
            }, 5000);
            
        } catch (error) {
            console.error('Error testing post-send actions:', error);
        }
        
        console.log('=== End Post-Send Action Test ===');
    },

    /**
     * Test post-send action immediately (for debugging)
     */
    async testPostSendActionNow() {
        console.log('=== Testing Post-Send Action Immediately ===');
        ...
        console.log('Triggering post-send action now...');
        await window.wasScheduler.handlePostSendAction();
        
        console.log('=== End Immediate Post-Send Action Test ===');
    },
     ...
     */
    async testTabClose() {
        console.log('=== Testing Tab Close Functionality ===');
        console.warn('⚠️ WARNING: This will close the current tab in 5 seconds!');
        console.log('Cancel if you have unsaved work...');
        ...
        setTimeout(async () => {
            console.log('Attempting to close tab now...');
            await window.wasScheduler.closeTab();
        }, 5000);
        
        console.log('=== Tab will close in 5 seconds ===');
    },
};
