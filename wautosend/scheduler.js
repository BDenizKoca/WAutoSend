...
class WASScheduler {    constructor() {
    ...
    async sendScheduledMessage(schedule) {
        ...
        try {
            ...
            if (success) {
                ...
                // Handle post-send action (refresh, close, or none)
                await this.handlePostSendAction();
            } else {
    ...
    async sendToTargets(messageText, contacts = []) {
        ...
        try {
            ...
            // Handle post-send action if message was sent successfully
            if (success) {
                await this.handlePostSendAction();
            }
        } catch (e) {
    ...
    /**
     * Handle post-send action after successful message send
     * Actions: 'refresh' (reload page), 'close' (close tab - avoids read confirmations & online status), 'none' (do nothing)
     */
    async handlePostSendAction() {
        try {
            this.log('Executing post-send action...');
            
            // Get settings to check post-send action
            const settings = await window.wasStorage.getSettings() || {};
            const action = settings.postSendAction || 'refresh'; // Default to refresh
            
            this.log(`Post-send action setting: ${action}`);
            
            switch (action) {
                case 'refresh':
                    this.log('Post-send action: refresh - refreshing page in 3 seconds...');
                    setTimeout(() => {
                        this.log('Auto-refreshing page to avoid read confirmations');
                        window.location.reload();
                    }, 3000);
                    break;
                    
                case 'close':
                    this.log('Post-send action: close - closing tab in 3 seconds...');
                    setTimeout(async () => {
                        this.log('Auto-closing tab to avoid read confirmations and online status');
                        await this.closeTab();
                    }, 3000);
                    break;
                    
                case 'none':
                    this.log('Post-send action: none - staying in current chat');
                    break;
                    
                default:
                    this.log('Unknown post-send action, defaulting to refresh');
                    setTimeout(() => {
                        window.location.reload();
                    }, 3000);
                    break;
            ...
            this.log('Error checking post-send action setting:', error);
        }
    }

    /**
     * Close the current tab using extension API
     * @returns {Promise<boolean>} Success status
     */
    async closeTab() {
        try {
            this.log('Attempting to close tab...');
            
            // Check if chrome.runtime is available
            if (!chrome || !chrome.runtime || !chrome.runtime.sendMessage) {
                this.log('Chrome runtime API not available, trying window.close()');
                window.close();
                return false;
            }
            
            const response = await chrome.runtime.sendMessage({ action: 'closeTab' });
            if (response && response.success) {
                this.log('Tab closed successfully');
                return true;
            } else {
                this.log('Tab close request failed:', response?.error || 'Unknown error');
                // Fallback: try window.close() anyway (might work in some cases)
                this.log('Falling back to window.close()');
                window.close();
                return false;
            }
        } catch (error) {
            this.log('Error closing tab:', error);
            // Fallback: try window.close() anyway
            this.log('Falling back to window.close()');
            window.close();
            return false;
        }
