...
class WASStorage {
        ...
        try {
            ...
            if (!settings) {
                await this.saveSettings({
                    ...
                    sendDelay: 3000,
                    autoRetry: true,
                    postSendAction: 'refresh' // Default to refresh behavior
                });
            } else if (settings.postSendAction === undefined) {
                // Migrate existing users to use refresh as default
                settings.postSendAction = 'refresh';
    ...
    async saveSetting(key, value) {
        try {
            ...
            console.error('WAS: Error saving setting:', error);
            return false;
        }
    }
    ...
    async getSetting(key) {
        try {
            ...
            console.error('WAS: Error getting setting:', error);
            return null;
        }
    }    /**
...
window.wasStorage.initializeDefaults().catch(error => {
    console.error('[WAS Storage] Failed to initialize defaults:', error);
});
